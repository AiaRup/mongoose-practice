...
mongoose.connect('mongodb://localhost/mongoose-practice', { useMongoClient: true })
...
Book.find({ pages: { $gt: 200, $lt: 500 } }, (err, bookResult) => {
  if(err) { return console.error(err); }
  console.log(bookResult);
});

//2. Find books whose rating is less than 5, and sort by the author's name
Book.find({ rating: { $lt: 5 } }).sort({ author: 'ascending' }).exec((err, bookResult) => {
  if (err) return console.error(err);
  console.log(bookResult);
});

//3. Find all the Fiction books, skip the first 2, and display only 3 of them
Book.find({ genres: 'Fiction' }).skip(2).limit(3).exec((err, bookResult) => {
  if (err) return console.error(err);
  console.log(bookResult);
});
...
Person.find({ height: { $gt: 180 }, salary:{ $gt: 30000 } }, (err, personResult) => {
  if (err) throw err;
  console.log(personResult);
});

//2. Find all the people who are tall (>180) OR rich (>30000)
Person.find({ $or:[{ height: { $gt: 180 } }, { salary:{ $gt: 30000 } }] }, (err, personResult) => {
  if (err) throw err;
  console.log(personResult);
});

//3. Find all the people who have grey hair or eyes, and are skinny (<70)
Person.find({ $or:[{ eyes: 'grey' }, { hair: 'grey' }], weight: { $lt: 70 } }, (err, personResult) => {
  if (err) throw err;
  console.log(personResult);
});

//4. Find people who have at least 1 kid with grey hair
Person.find({ kids: { $elemMatch: { hair: 'grey' } } }, (err, personResult) => {
  if (err) throw err;
  console.log(personResult);
});
...
Person.find().and([
  ...
  { weight: { $gt: 100 } }]).exec((err, personResult) => {
  if (err) throw err;
  console.log(personResult);
});
